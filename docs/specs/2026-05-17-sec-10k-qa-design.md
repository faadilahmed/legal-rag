...
- **`preprocess.py` section detection — three interacting fixes applied during T4 smoke-testing:**
  1. **Keep `<table>` tags.** SEC filings render prose (including Risk Factors content) inside HTML tables for layout. Dropping `<table>` lost most substantive content (AAPL: 184k → 222k chars after the fix).
  2. **Anchor `ITEM_PATTERN` to line start (`^\s*Item ...` with MULTILINE).** Without anchoring, mid-sentence cross-references like *"…read in conjunction with Part II, Item 7, 'Management's Discussion'…"* matched and prematurely terminated real section spans (AAPL Item 1A: 790 → 68,048 chars).
  3. **Dedup by `(score, length)` with a 100k-char span cap.** For each unique item number, keep the candidate with the highest title score (real headings starting with capital words beat cross-reference fragments starting with "of"/"in"/punctuation), then longest text span. Skip any candidate whose span exceeds 100k chars (catches cases where a cross-reference at the end of a sparse region sweeps up multiple following sections — saw JPM Item 1A claiming 600k+ chars before this).
  Verified across AAPL/MSFT/JPM/XOM/WMT/JNJ/GOOGL/TSLA: 7 of 8 have Item 1A in the expected 6k–85k range. JPM remains an edge case (its filing references Items in a structure that doesn't match standard 10-K layout); JPM's full 1.4M chars are still chunked, just with imperfect section labels. Embedding-based retrieval still works because chunk content is faithful even when section metadata is wrong.
